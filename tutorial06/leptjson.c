        ...
        switch (ch) {
            case '\"':
                *len = c->top - head;
                ...
                /* strncpy(*str, (const char *)lept_context_pop(c, *len), *len); strncopy STOPS after a null character */
                c->json = p;
                return LEPT_PARSE_OK;
            case '\\':
                switch (*p++) {
...
static int lept_parse_object(lept_context* c, lept_value* v) {
    ...
    free(m.k);
    for (i = 0; i < size; i++) {
        lept_member* _m = (lept_member*)lept_context_pop(c, sizeof(lept_member));
        free(_m->k);
        lept_free(&_m->v);
    }
    v->type = LEPT_NULL;
